...
def mutate(ind,num=random.randint(1,2)):
...
def pfga():
    ...
    if len(population) < 2:
        try:
            individual1, individual2 = crossover_1(population[0])
            population.append(individual1)
            population.append(individual2)
        except:
            individual1, individual2 = crossover_0()
            population.append(individual1)
            population.append(individual2)
...
with open('PfGA_result.csv','w') as fout:
    ...
    while True:
        ...
        gen+=1
        # 終了条件：誤差率が--になるまで
        if (record_distance - att48)/record_distance <= 0.30:
